...
export const ChatListUtils = {
  listKey: '_chatList',
  //初始化群组列表
  initChatList: function(that,userId) {
    if (userId==null){
      console.log("初始化会话列表失败,用户ID参数丢失");
    } ;
    let requestApi = RequestUtils.getInstance();
    let params={"userId":userId,"groupName":''};
    requestApi.request(conf.getMyChatSessionListUrl(), params).then(res => {
      if (res.code==1&&res.data!=null){
        let chatList=res.data;
        //过滤掉系统群组 groupType==3
        let newChatList = chatList.filter(function(element) {
          return element.groupType !== 3;
        });
        that.$store.commit("setChatList",newChatList);
      }else{
        console.log("初始化会话列表失败");
      }
    }).catch(function(error) {
      console.log("初始化会话列表异常",error);
    });
  },
  ...
  /**修改聊天会话*/
  updateChat:function(userId,chat){
    let tempChatList = [];
   ...
   * @returns {Chat} 当前会话
   */
  addNewChat: function(self, newChat) {
    console.log('新的会话:' + JSON.stringify(newChat));
    console.log("发起聊天的当前用户",self.$store.state.user.id);
    let currentUserId=self.$store.state.user.id;
    let chatList = this.getChatList(currentUserId);
    console.log("原有的会话列表",chatList);
    // 删除当前用户已经有的会话(去除重复)
    let newChatList = chatList.filter(function(element) {
      return String(element.chatGroupId) !== String(newChat.chatGroupId);
    });
    // 重新添加会话，放到第一个
    let chat = new Chat(newChat);
    newChatList.unshift(chat);
    console.log("新的会话列表",newChatList);
    // 存储到localStorage 的 chatList
    this.setChatList(self.$store.state.user.id, newChatList);
    //改变全局管理中的的临时会话列表
...
export function selfAlert(tip,topHeight,duringTime,type,that){
